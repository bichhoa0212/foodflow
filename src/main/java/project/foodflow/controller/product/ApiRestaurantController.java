...
public class ApiRestaurantController {
    ...
    public ResponseEntity<Response<java.util.List<project.foodflow.dto.ProductDto>>> getProductsByRestaurant(@PathVariable Long id) {
        java.util.List<project.foodflow.entity.Product> products = productRepository.findByRestaurantId(id);
        java.util.List<project.foodflow.dto.ProductDto> dtos = products.stream().map(product -> {
        ...
        return ResponseEntity.ok(new Response<>(project.foodflow.constant.ReturnCode.SUCCESS.getCode(), project.foodflow.constant.ReturnCode.SUCCESS.getStatus(), "Lấy danh sách món ăn của nhà hàng thành công", dtos));
    }
